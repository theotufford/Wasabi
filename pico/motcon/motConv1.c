...
int dispenseReagent(Machine machine, char* inputData){
  ...
  while (tok != NULL) {  
    ...
    for (int i=0; i < machine.pumpCount; i++){
      char* pumpReagent= machine.pumps[i] -> reagent;
...
int main() {
  ...
  */
  ...
  // main control loop
  for (int i = 0; i < 200; i++){
    gpio_put(driver1.stepPin, 1);
    sleep_us(1);
    gpio_put(driver1.stepPin, 0);
//    step(wasabi.bMot);
//    stepPump(wasabi.pumps[0]);
    sleep_us(1000);
  }
//  char inputBuf[64];
//  while (1) {
...
//      dispenseReagent(wasabi, cleanedInput);
//    }
